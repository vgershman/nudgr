...
class PendingIntent(Base):
    """v2.5: in-flight reminder intent waiting on a user clarification.

    When the parser returns needs_clarification (e.g. "remind me to test feature"
    with no time), we stash what we know here and ask the user. The next
    free-form message from the same user is parsed WITH this context, so the
    partial info merges instead of being forgotten.

    One row per user (PK). Newer pendings overwrite older. Auto-expires at
    `expires_at` (default 15 min); a sweeper purges stale rows.
    """
    ...
    chat_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    clarification_message_id: Mapped[int | None] = mapped_column(BigInteger)
    original_text: Mapped[str] = mapped_column(Text, nullable=False)
    # Shape: {"target_text": str, "recurrence": dict|None,
